...
result_folder=f'../../results_phase2_rl/{args.train}_{args.embedding}_{args.anchors}_{args.norm}_{model_name}_{num_layers}_{args.modelPhase2}_{args.iter_update_params}_{args.initial_type}_{args.initial_order}_{args.scoring_func}_{args.node_select}_{num_perturbation}_{args.node_select_heuristic}_{args.pool}_{args.update_reward}_{args.hops}'
core_node_path=f'../../results_phase1/{args.train}_{args.embedding}_{args.norm}_{model_name}_{num_layers}'


model_folder=f'../../models_phase2_rl/{args.train}_{args.embedding}_{args.anchors}_{args.norm}_{model_name}_{num_layers}_{args.modelPhase2}_{args.iter_update_params}_{args.initial_type}__{args.initial_order}_{args.scoring_func}_{args.node_select}_{num_perturbation}_{args.node_select_heuristic}_{args.pool}_{args.update_reward}_{args.hops}'
...
def getPartitions_rl(model,data,device,iter_update_params=5,store_norm_cuts=False,j=-1):
    start=time.time()
    with torch.no_grad():
        model.eval()           
        num_nodes = data.num_nodes
        ...
        actions=[]
        logits=[]
        init_cut = getNormalisedCutValue(data,partitions,data.num_cuts ,device).item()
        if node_select:
            node_scores = get_node_scores(data,partitions,node_select_heuristic,hops,device)
        else:
            order = get_order(data,initial_order)
        ...
        for i in range(num_perturbation_inference):
            # curr_node_id = int(data.x[1][i].item())            # data.x[1] is the node_ordering 
            if node_select:
                # sample from node_scores
                curr_node_id = torch.tensor(torch.multinomial(node_scores,1).item())
    ...
    last_cut = getNormalisedCutValue(data, partitions, data.num_cuts ,device,default=3*init_cut).item()
...
def calculate_stats(dataset,type):   
    ...
    for j in range(len(dataset)):
        ...
        for _ in range(num_run): ## TODO how many runs
            ...
            partitions,norm_cut = getPartitions_rl(model=model, data=data, device=device, iter_update_params=args.iter_update_params,store_norm_cuts=True,j=j)
            if (best_norm_cut==-1 or norm_cut < best_norm_cut) and (not math.isnan(norm_cut)):
                best_norm_cut = norm_cut
                best_partitions = partitions
