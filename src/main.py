import torch
import os
from model import ModelBasic, ModelLinkPred, ModelAtten, ModelLocalLinkPred, ModelLinkPred_weight
...
parser.add_argument('--num_perturbation',type=float,default=1, help='Number of perturbations for each node')
...
parser.add_argument('--lr',type=float,default=0.0005, help='value of lr')
...
norm=args.norm
gamma=args.gamma ##discount factor TODO: Make it a parameter to be tuned by RL 
...
result_folder=f'../results/{args.train}_{args.embedding}_{args.anchors}_{args.norm}_{model_name}_{num_layers}_{args.modelScore}_{args.iter_update_params}_{args.initial_type}_{args.initial_order}_{args.scoring_func}_{args.node_select}_{num_perturbation}_{args.node_select_heuristic}_{args.pool}_{args.update_reward}_{args.hops}_{args.cuttype}_{args.gamma}_{args.alpha_reward}_{args.beta}_{args.lr}'

print("Result Folder: ", result_folder)

if wandb_flag:
    wandb.init(project="phase2_rl", name=f"{args.train}_{args.embedding}_{args.anchors}_{args.norm}_{model_name}_{num_layers}_{args.modelScore}_{args.iter_update_params}_{args.initial_type}_{args.initial_order}_{args.scoring_func}_{args.node_select}_{num_perturbation}_{args.node_select_heuristic}_{args.pool}_{args.update_reward}_{args.hops}_{args.cuttype}_{args.gamma}_{args.alpha_reward}_{args.beta}_{args.lr}", config=args,entity='mincuts')

model_folder=f'../models/{args.train}_{args.embedding}_{args.anchors}_{args.norm}_{model_name}_{num_layers}_{args.modelScore}_{args.iter_update_params}_{args.initial_type}__{args.initial_order}_{args.scoring_func}_{args.node_select}_{num_perturbation}_{args.node_select_heuristic}_{args.pool}_{args.update_reward}_{args.hops}_{args.cuttype}_{args.gamma}_{args.alpha_reward}_{args.beta}_{args.lr}'
...
elif args.modelScore=='ModelLinkPred_weight':
    model = ModelLinkPred_weight(num_features=train_dataset.num_no_features, num_layers = num_layers, model_name = model_name, hidden_channels=32 ,scoring_func=scoring_func,device=device).to(device)   # Note change num_features when needed
else:
...
def get_reward(prev_cut,new_norm_cut):
    ...
    # if modularity -> (new_norm_cut - prev_cut)* e^(min(new_norm_cut),2))*alpha
    if cuttype=='modularity':
        reward = (new_norm_cut-prev_cut)*math.exp(min(alpha_reward*new_norm_cut,3))*1000
    else:
...
def train(epoch):  
    ...
    for _,data in enumerate(dataloader):
        ...
        for i in tqdm(range(int(num_nodes*num_perturbation))):
            ...
            # print("logits",logits)
            if (i%args.iter_update_params==0 and i!=0) or  i==int(num_nodes*num_perturbation)-1:
...
def getPartitions_rl(model,data,device,iter_update_params=5,store_norm_cuts=False):
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(int(num_nodes*num_perturbation))):
            ...
            # print("logits",logits)
            # # exit(0)
            if (i%iter_update_params==0 and i!=0) or  i==int(num_nodes*num_perturbation)-1:
                actions=[]
                logits=[]
    ...
    for j in range(len(dataset)):
        ...
        for _ in range(num_run): ## TODO how many runs
            ...
            if (best_norm_cut==-1 or norm_cut < best_norm_cut) and (not math.isnan(norm_cut)):
                ...
                inference_time = tot_time
        
        if wandb_flag:
            print("here")
            wandb.log({f"hMetis NormCut {type}":data.hMetis_norm_cut,f"spectral NormCut {type}":data.spectral_norm_cut,f"Best NormCut {type}":best_norm_cut,f"Inference Time {type}":inference_time})
...
if __name__ == '__main__':
    ...
    if (need_training):
        ...
        for epoch in tqdm(range(num_epochs)):  # loop over the dataset multiple times
            # print('epoch ', epoch + 1)
            reward,init_cut,last_cut= train(epoch)
            rewards.append(reward)
